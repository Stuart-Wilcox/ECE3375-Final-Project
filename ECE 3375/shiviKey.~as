...
RFID:	 	  CLRB  			  			  ;clear accumulator b
			  CLRA			  			  ;clear accumulator a
			  LDX Stored_RFID_Data
Recieving: 	  BRCLR SCI0SR1,$20,*  		 ;Check when we are completed recieving, spin other wise
			  LDAA SCI0DRL 				 ;load the data in the reciving data register 
			  CMPA ,X+
			  BNE ERROR					 ;means we didnt find a match in data and we have wrong ID input
			  INCB
			  CMPB #$08					 ;Chcek to see if we have hit the last byte of data
			  BEQ NOERROR
			  BRA Recieving
ERROR:        JSR HReset				 ;LED goes off, send signal of 0 
			  BRA CHECK_SW
			  	  						 ;loop back to the beginning of code and check for user input again			  
NOERROR:   	 LDAA #%11111111 			 ;LED goes on, send signal of 1
		   	 STAA PORTA					 ;loop back to the beginning of code and check for user input again
			 LDAB #%11111111			 ;Kepp LED on for some time
SNOERROR:	 DECB
			 BEQ Return
			 JSR Delay
			 BRA SNOERROR
Return:		 LDAA #%00000000			 ;Reset system
			 STAA PORTA
			 JSR HReset
			 BRA CHECK_SW
